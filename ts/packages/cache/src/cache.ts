...
export default class Cache {
  readonly #cache = new Map<ID_of<Object>, WeakRef<Object>>();
  readonly cacheId = cacheId++;
  readonly #finalizationRegistry: FinalizationRegistry<ID_of<Object>>;

  constructor() {
    this.#finalizationRegistry = new FinalizationRegistry(
      (cacheKey: ID_of<Object>) => {
  ...
  get<T extends Object>(
    ...
    const ref = this.#cache.get(idconcat as ID_of<T>);
    if (ref == null) {
      return null;
    }
...
function concatId<T>(id: ID_of<T>, db: string, tablish: string) {
  return (id + "-" + db + "-" + tablish) as ID_of<T>;
}
