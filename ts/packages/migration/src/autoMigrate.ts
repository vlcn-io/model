...
async function migrateAllTasksForDB(db: SQLResolvedDB, tasks: MigrationTask[]) {
  try {
    ...
    throw {
      cause: e,
      message:
        "Failed to commit the migration. Rolling it back. " +
        (e as any)?.message,
    };
  }
}
...
export async function getOldSql(
  db: SQLResolvedDB,
  tableName: string
): Promise<string> {
  ...
  if (rows.length < 1) {
    throw new Error(
      "Could not find the old schema for " +
        tableName +
  ...
  if (rows.length > 1) {
    throw new Error(
      "Unexpected number of old schemas returned by the database for table " +
        tableName
    );
  }
...
export function extractColumnDefs(sql: string): ColumnDef[] {
  ...
  if (match == null) {
    throw new Error(
      "could not extract column definitions from the sql create table clause: " +
        sql
    );
  }
  ...
  return match[1]
    ...
    .map((c) => {
      const match = c.match(
        /^"(?<name>.*?)"\s*(?<nonnull>NOT NULL)?\s*(\/\*(?<meta>.*)\*\/)?/
      );
...
export function findRemovedColumns(
  ...
  const allNumbered =
    left.every((c) => c.num != null) && right.every((c) => c.num != null);
  if (!allNumbered) {
 ...
 * @returns
 */
export function findAddedColumns(
  left: ColumnDef[],
  right: ColumnDef[]
): ColumnDef[] {
  const allNumbered =
...
function removeStatements(tableName: string, columns: ColumnDef[]): SQLQuery[] {
  return columns.map(
    (c) =>
      sql`ALTER TABLE ${sql.ident(tableName)} DROP COLUMN ${sql.ident(c.name)}`
  );
}
