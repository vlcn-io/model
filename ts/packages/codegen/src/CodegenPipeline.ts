...
export default class CodegenPipleine {
  ...
  async gen(
    ...
    const globalStepFiles = (
      await Promise.all(
        this.globalSteps.map((step) =>
          !step.accepts(nodes, edges)
            ? null
            : new step(nodes, edges, "domain.js").gen()
        )
      )
    ).filter((f): f is CodegenFile => f != null);

    const allFiles = [...nodeAndEdgeFiles, ...globalStepFiles].filter(
      (f) => f.nochange !== true
    );
    ...
    await Promise.all(
      toWrite.map(
        async (f) => await fs.promises.writeFile(toPath(dest, f[0]), f[1])
      )
    );
  }
  ...
  private async checkHashesAndAddManualCode(
    ...
    return await Promise.all(
      files.map(async (f) => {
        ...
        try {
          const contents = await fs.promises.readFile(toPath(dest, f.name), {
            encoding: "utf8",
          });
          try {
