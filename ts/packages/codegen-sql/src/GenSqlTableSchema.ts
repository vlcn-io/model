...
export default class GenSqlTableSchema extends CodegenStep {
  ...
  private getSqliteString(): string {
    ...
    const columnDefs = Object.values(this.schema.fields).map((field) => {
      ...
      if (field.num != null) {
        ret = sql`${ret} /* n=${sql.__dangerous__rawValue(
          field.num.toString()
        )} */`;
      }
      return ret;
    ...
    const createTable = sql`CREATE TABLE ${sql.ident(
      nodeFn.tableName(this.schema)
    )} (${sql.join(columnDefs, ", ")})`.format(
      formatters[this.schema.storage.engine]
    ).text;
  ...
  private getPostgresString(): string {
    ...
    if (this.schema.type === "node" && this.schema.primaryKey) {
      columnDefs.push(
        sql`CONSTRAINT ${sql.ident(
          tableName + "_pkey"
        )} PRIMARY KEY (${sql.ident(this.schema.primaryKey)})`
      );
    }
...
function extractTypeAtomsForSQL(field: FieldDeclaration): [TypeAtom, boolean] {
  ...
  if (type.length === 0) {
    throw new Error(
      `All fields must have types. No types found for field ${field.name}`
    );
  }
  ...
  if (itemType == null) {
    throw new Error(
      `Could not find a type for ${field.name} that was not "|", "&", or "null"`
    );
  }
